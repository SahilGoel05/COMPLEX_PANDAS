...
function TaskDetailsModal({ task, onClose, updateTask }) {
    ...
    const [editedDueDate, setEditedDueDate] = useState(new Date(task.duedate).toISOString().substr(0, 10));
                ...
                <div className="task-detail">
                    ...
                    {isEditingDueDate ? (
                        ...
                        <span onClick={() => setIsEditingDueDate(true)}>
                            {new Date(editedDueDate).toLocaleDateString()} <button className="edit-button">✎</button>
                        </span>
                    )}
                </div>
                <div className="task-detail">
