...
describe("CategorySidenav Component", () => {
  ...
  const mockFetchCategories = jest
    .fn()
    .mockResolvedValue({ data: mockCategories });
  ...
  test("handles adding a new category", async () => {
    ...
    fireEvent.change(await findByPlaceholderText("Enter new category name"), {
  ...
  test("handles selecting a category", async () => {
    ...
    await waitFor(() =>
      expect(mockSetSelectedCategory).toHaveBeenCalledWith("2"),
    );
  });
  ...
  test("cancels adding a new category", async () => {
    ...
    await waitFor(() =>
      expect(queryByPlaceholderText("Enter new category name")).toBeNull(),
    );
  });
